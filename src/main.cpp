...
static void btserial_task(void *pvParameter)
{
    ...
    while (1)
    {
        ...
        else if (opt.misc.btMsgType == BT_MSG_XCTRC)
        {
...
#elif USE_BMP388
        if (baroCounter >= 10)
        { // 10*2mS = 20mS elapsed, this is the configured sampling period for BMP388
            baroCounter = 0;
            bmp388_sample();
            // KF4 uses the acceleration data in the update phase
            float zAccelAverage = ringbuf_averageNewestSamples(10);
...
void setup()
{
    ...
    audio_config(pinAudioDAC);
    audio_setFrequency(1000);
    ESP_LOGD(TAG, "Set Frequency");
